    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center flex-1 w-full mx-auto px-6 md:px-14">
        ...
        <motion.h1
          ...
          className="text-center text-white font-semibold w-full"
          style={{ fontSize: "clamp(2.6rem, 8vw, 100px)", lineHeight: 1.0 }}
        >
          Win More Clients.
          <br />
        ...
        <motion.p
          ...
          className="text-center text-north-gray font-normal max-w-[900px] w-full mt-5"
          style={{ fontSize: "clamp(0.95rem, 1.6vw, 18px)", lineHeight: 1.6 }}
        >
        ...
        >
          <Button
            ...
            className="rounded-full bg-north-gold text-north-navy hover:bg-north-gold/90 font-semibold text-[15px] h-[52px] px-9 shadow-none transition-transform hover:scale-[1.02]"
          >
            Get Started Free
          </Button>
          ...
          <Button
            variant="ghost"
            className="rounded-full border border-north-gold/80 bg-transparent text-white hover:bg-north-gold/15 hover:text-white font-semibold text-[15px] h-[52px] px-9"
            onClick={() => {
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-north-gray font-medium text-[14px]">Trusted by 2,000+ professionals</p>
          </div>
        </motion.div>
      </div>

      {/* Testimonial marquee at bottom */}
      <div className="relative z-10 w-full overflow-hidden py-6 border-t border-north-gold/25">
        <Marquee gradient={false} speed={35} pauseOnHover>
          <div className="flex items-center shrink-0 gap-8 pr-8">
            {HERO_TESTIMONIALS.map((t, i) => (
              <div key={i} className="flex items-center gap-3 bg-north-gray/10 backdrop-blur-sm rounded-full py-2.5 px-5 shrink-0 border border-north-gold/15">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={t.avatar}
                  alt={t.name}
                  className="w-9 h-9 rounded-full object-cover border-2 border-north-gold/40 shrink-0"
                />
                <div className="flex flex-col min-w-0">
                  <p className="text-north-gray italic text-[13px] leading-tight truncate max-w-[220px]">
                    &ldquo;{t.quote}&rdquo;
                  </p>
                  <span className="text-white/70 text-[11px] font-medium mt-0.5">{t.name}</span>
                </div>
              </div>
            ))}
          </div>
